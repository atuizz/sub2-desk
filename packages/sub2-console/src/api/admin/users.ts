...
export async function updateBalance(
  ...
  const expected = { owner: JSON.parse(localStorage.getItem('auth_user') || 'null')?.id || 0, token: localStorage.getItem('auth_token') }
  const { captureIncrementalOwner, incrementalWriteJournal } = await import('../../utils/incrementalWriteGuard')
  return incrementalWriteJournal.run(captureIncrementalOwner(expected), 'balance', id, async key => {
    ...
    return data
  }, value => validBalanceResult(value, id))
}

const validBalanceResult = (value: AdminUser, id: number) => value?.id === id && typeof value.balance === 'number' && Number.isFinite(value.balance)
export async function createBalanceWriteGuard() {
  const { createIncrementalWriteGuard } = await import('../../utils/incrementalWriteGuard')
  return createIncrementalWriteGuard('balance', getById, validBalanceResult, value => `当前余额：$${value.balance.toFixed(2)}`)
}
...
// Attribute values are saved independently of the basic user profile.
export async function listAttributeDefinitions(enabledOnly = true): Promise<import('../../types/admin-policies').AttributeDefinition[]> {
  const { data } = await apiClient.get<import('../../types/admin-policies').AttributeDefinition[]>('/admin/user-attributes', enabledOnly ? { params: { enabled: true } } : undefined)
  return data
}
export async function getAttributeValues(id: number): Promise<import('../../types/admin-policies').AttributeValue[]> {
  const { data } = await apiClient.get<import('../../types/admin-policies').AttributeValue[]>(`/admin/users/${id}/attributes`)
  return data
}
export async function updateAttributeValues(id: number, values: Record<number, string>): Promise<{ message: string }> {
  const { data } = await apiClient.put<{ message: string }>(`/admin/users/${id}/attributes`, { values })
  return data
}
export async function createAttributeDefinition(input: import('../../types/admin-policies').AttributeDefinitionInput): Promise<import('../../types/admin-policies').AttributeDefinition> {
  const { data } = await apiClient.post<import('../../types/admin-policies').AttributeDefinition>('/admin/user-attributes', input)
  return data
}
export async function updateAttributeDefinition(id: number, input: Partial<import('../../types/admin-policies').AttributeDefinitionInput>): Promise<import('../../types/admin-policies').AttributeDefinition> {
  const { data } = await apiClient.put<import('../../types/admin-policies').AttributeDefinition>(`/admin/user-attributes/${id}`, input)
  return data
}
export async function deleteAttributeDefinition(id: number): Promise<{ message: string }> {
  const { data } = await apiClient.delete<{ message: string }>(`/admin/user-attributes/${id}`)
  return data
}
export async function reorderAttributeDefinitions(ids: number[]): Promise<{ message: string }> {
