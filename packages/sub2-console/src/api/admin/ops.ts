...
export function subscribeQPS(onMessage: (data: any) => void, options: SubscribeQPSOptions = {}): () => void {
  ...
  const scheduleReconnect = () => {
    if (!shouldReconnect) return
    if (reconnectAttempts >= maxReconnectAttempts) { setStatus('closed'); return }
  ...
  const connect = () => {
    ...
    if (ws && (ws.readyState === WebSocket.OPEN || ws.readyState === WebSocket.CONNECTING)) return
    // Initial connection is allowed even when retries are disabled. Retry budget
    // applies before scheduling, including failures before the first open.
    ...
    ws.onopen = () => {
      if (!shouldReconnect || ws !== socket) return
      reconnectAttempts = 0
      isConnecting = false
