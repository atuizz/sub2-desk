...
/** Same read-only account search contract used by the official Codex manifest selector. */
export async function searchManifestAccounts(groupId: number, search: string, page = 1): Promise<{items:{id:number;name:string}[];total:number}> {
  const { data } = await apiClient.get<{items:{id:number;name:string}[];total:number}>('/admin/accounts', {
    params: { page, page_size: 20, search, platform: 'openai', group: String(groupId) }
  })
  return { items: data.items.map(({id,name}) => ({id,name})), total: data.total }
}
...
// Retain the existing desktop API name while using the official endpoint.
