 ...
 * Get current version
 */
export async function getVersion(signal?: AbortSignal): Promise<{ version: string }> {
  const { data } = await apiClient.get<{ version: string }>('/admin/system/version', { signal })
  return data
}
 ...
 */
export async function checkUpdates(force = false, signal?: AbortSignal): Promise<VersionInfo> {
...
export interface UpdateResult {
  message: string
  // An already-current update response omits need_restart.
