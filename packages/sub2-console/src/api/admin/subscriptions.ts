...
export async function extend(
  ...
  // Copy before acquiring the cross-window lock: edits must not change this logical request.
  ...
  return incrementalWriteJournal.run(captureIncrementalOwner(expected), 'subscription', id, async key => {
    const { data } = await apiClient.post<UserSubscription>(
      `/admin/subscriptions/${id}/extend`, payload, { headers: { 'Idempotency-Key': key } }
    )
    return data
  }, value => validExtensionResult(value, id))
}

const validExtensionResult = (value: UserSubscription, id: number) => value?.id === id && typeof value.expires_at === 'string' && Number.isFinite(Date.parse(value.expires_at))
export async function createExtensionWriteGuard() {
  const { createIncrementalWriteGuard } = await import('../../utils/incrementalWriteGuard')
  return createIncrementalWriteGuard('subscription', getById, validExtensionResult, value => `当前到期：${new Date(value.expires_at || '').toLocaleString()}`)
}
