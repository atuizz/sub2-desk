 ...
 */
export async function create(accountData: CreateAccountRequest, idempotencyKey?: string): Promise<Account> {
  // The caller retains the key for one logical write. Auth transport retries keep this header.
  const key = idempotencyKey ?? `account-create-${globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(36).slice(2)}`}`
  const { data } = await apiClient.post<Account>('/admin/accounts', accountData, { headers: { 'Idempotency-Key': key } })
  return data
}
...
function validateGrokMediaEligibility(id: number, data: unknown): GrokMediaEligibilityState {
  const state = data as Partial<GrokMediaEligibilityState> | null
  if (!state || typeof state !== 'object' || Array.isArray(state) || state.account_id !== id ||
      typeof state.mode !== 'string' || !['auto', 'enabled', 'disabled'].includes(state.mode) || typeof state.eligible !== 'boolean' || typeof state.reason !== 'string') {
    throw new Error('媒体资格响应无效或不属于当前账号，请重新读取。')
  }
  return { account_id: id, mode: state.mode as GrokMediaEligibilityMode, eligible: state.eligible, reason: state.reason }
}

/** Official dedicated endpoint; never replace account.extra to change media eligibility. */
export async function getGrokMediaEligibility(id: number): Promise<GrokMediaEligibilityState> {
  ...
  return validateGrokMediaEligibility(id, data)
}

export async function updateGrokMediaEligibility(id: number, mode: GrokMediaEligibilityMode): Promise<GrokMediaEligibilityState> {
  if (!Number.isSafeInteger(id) || id <= 0) throw new Error('账号 ID 无效。')
  if (!['auto', 'enabled', 'disabled'].includes(mode)) throw new Error('请选择有效的媒体资格模式。')
  const { data } = await apiClient.put<GrokMediaEligibilityState>(`/admin/accounts/${id}/grok-media-eligibility`, { mode })
  return validateGrokMediaEligibility(id, data)
}
