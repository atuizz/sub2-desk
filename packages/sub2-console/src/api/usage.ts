...
export async function getDashboardModels(params?: {
  ...
  billing_mode?: string | null
  timezone?: string
}, config: { signal?: AbortSignal } = {}): Promise<ModelStatsResponse> {
...
export async function getMyErrorDetail(id: number, config: { signal?: AbortSignal } = {}): Promise<UserErrorRequestDetail> {
  const { data } = await apiClient.get<UserErrorRequestDetail>('/usage/errors/' + id, config)
  return data
}
