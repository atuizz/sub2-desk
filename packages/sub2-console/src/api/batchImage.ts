...
async function parseBatchImageError(response: Response): Promise<Error> {
  try {
    ...
    const candidates = [body?.error?.message, body?.message, body?.detail, typeof body?.error === 'string' ? body.error : undefined]
    const message = candidates.find(value => typeof value === 'string' && value.trim()) || response.statusText || `HTTP ${response.status}`
