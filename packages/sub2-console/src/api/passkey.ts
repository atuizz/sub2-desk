...
async function login(proof?: ActionCaptchaRequestProof, signal?: AbortSignal): Promise<AuthResponse> {
...
async function register(name: string, password: string, signal?: AbortSignal, onFinish?: () => void): Promise<PasskeyCredentialSummary> {
  requirePasskeySupport()
  signal?.throwIfAborted()
