...
import { AUTH_LINEAGE_EVENT, accessTokenMatchesSession, publishTokenRotation, storedTokenSessionID } from './authTokenLineage'
...
function readAuthSnapshot(): AuthSnapshot {
  ...
  return {
    ...
    sessionID: storedTokenSessionID()
  }
}
...
function readPeerRefreshResult(
  ...
  if (!storedPair || storedTokenSessionID() !== snapshot.sessionID ||
      !accessTokenMatchesSession(snapshot.userID, failedAccessToken || snapshot.accessToken)) {
    return null
  }
...
async function requestTokenPair(
  ...
  try {
    ...
    if (!payload.data.access_token?.trim() || !payload.data.refresh_token?.trim()) throw new Error('Invalid refresh response')
    publishTokenRotation(snapshot.userID, snapshot.accessToken, snapshot.refreshToken, payload.data.access_token, payload.data.refresh_token)
    persistTokenPair(payload.data)
    window.dispatchEvent(new Event(AUTH_LINEAGE_EVENT))
    return payload.data
  } catch (error) {
...
async function runRefresh(options: RefreshAuthTokensOptions): Promise<RefreshTokenResponse> {
  ...
  if (options.failedAccessToken && !accessTokenMatchesSession(snapshot.userID, options.failedAccessToken)) {
    throw new Error('Session changed before token refresh')
  }
