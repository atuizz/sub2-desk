async (page) => {
  ...
  for(const app of apps){
    ...
    await page.screenshot({path:'output/playwright/polish/apps/'+app.id+'-light.png',animations:'disabled'});
  }
  failure=false;
  for(const id of ['dashboard','accounts','finder','settings']){
    ...
    await page.screenshot({path:'output/playwright/polish/'+id+'-light.png',animations:'disabled'});
    await page.evaluate(()=>window.__macOS.systemStore.setAppearance('dark'));
    await page.screenshot({path:'output/playwright/polish/'+id+'-dark.png',animations:'disabled'});
  ...
  await page.screenshot({path:'output/playwright/polish/desktop-light.png',animations:'disabled'});
  ...
  await page.screenshot({path:'output/playwright/polish/launcher-light.png',animations:'disabled'});
  await page.keyboard.press('Escape');
  await page.getByRole('button',{name:'控制中心',exact:true}).click();
  await page.getByRole('region',{name:'控制中心',exact:true}).waitFor();
  await page.screenshot({path:'output/playwright/polish/control-center-light.png',animations:'disabled'});
  await page.keyboard.press('Escape');
  await page.setViewportSize({width:390,height:844});
  for(const id of ['settings','finder','dashboard','accounts']){
    await page.evaluate(id=>{const w=window.__macOS.wm;[...w.windows.value].forEach(a=>w.closeWindow(a.id));w.openApp(id);},id);
    await page.waitForTimeout(300);
    await page.screenshot({path:'output/playwright/polish/'+id+'-mobile.png',animations:'disabled'});
  }
  return {apps:checks,appCount:checks.length,pageErrors:errors,writes,fixtureOnly:true,apiPaths:[...new Set(apiRequests)]};
}
